...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if short code already exists for this organization
    ...
    const newUrl = await queries.createUrl({
      ...
      originalUrl: url,
      shortCode,
      domainId: domain ? null : null, // We'll implement custom domains later
    ...
    // Construct short URL
    const finalDomain = domain || process.env.DEFAULT_DOMAIN || 'localhost:3000';
